...
public class TombstonePatches
{
    ...
    private static class TombStoneAwakePatch
    {
        private static void Prefix(TombStone __instance)
        {
            ...
            int height = Layout.BaseInventoryHeight + ExtraRows.Value + (AddEquipmentRow.Value.isOn() ? API.API.GetAddedRows(__instance.GetComponent<Container>().m_width) : 0);
    ...
    private static class TombStoneInteractPatch
    {
        private static void Prefix(TombStone __instance, Container ___m_container)
        {
            ...
            int num = Layout.BaseInventoryHeight + ExtraRows.Value + (AddEquipmentRow.Value.isOn() ? API.API.GetAddedRows(__instance.GetComponent<Container>().m_width) : 0);
    ...
    private static class TemporarilyIncreaseCarryWeight
    {
        ...
        private static void Prefix(out int __state)
        {
            ...
            __state = AddEquipmentRow.Value.isOn() ? API.API.GetAddedRows(Player.m_localPlayer.m_inventory.m_width) : 0;
